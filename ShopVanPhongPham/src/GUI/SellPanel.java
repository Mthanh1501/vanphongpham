...
public class SellPanel extends JPanel {
	...
	public SellPanel(int id_user) {
		...
		panel.setBorder(new TitledBorder((Border) new EtchedBorder(EtchedBorder.LOWERED, null, null),
				"Ho\u00E1 \u0111\u01A1n", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
		add(panel);
		...
		btn_update.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (productList1.isEmpty() || maKhachHang1.isEmpty()) {
					...
					for (Product product : productList1) {
						InvoiceDao.insertDetailSell(aa, product.getMaSanPham(), product.getSoLuong(),
								product.ThanhTien());
					}
					...
					for (Product product : productList) {
						tableModel.addRow(
								new Object[] { product.getMaSanPham(), product.getMaLoai(), product.getTenSanPham(),
	...
	public void search(String item) {
		...
		try {
			Connection cons = DBConnect.getConnection();
			String sql = "SELECT Product.*,`Category`.`maLoai` idCategory, Provider.`maNhaCungCap` idProvider  from Product INNER JOIN Category ON  Product.`maLoai` = Category.`maLoai` INNER JOIN Provider 	ON Product.`maNhaCungCap` = Provider.`maNhaCungCap` where tenSanPham LIKE '%"
					+ item + "%'";
			PreparedStatement pr = cons.prepareStatement(sql);
			ResultSet r1 = pr.executeQuery();
			while (r1.next()) {
	...
	public void xuatpdf(int mahoadon) throws FileNotFoundException {
		...
		document.add(new Paragraph("Ma CT " + invoiceList.get(0).getMaHoaDon() + " " + invoiceList.get(0).getNgayban()
				+ " " + invoiceList.get(0).getNgayBan() + " NV " + invoiceList.get(0).getMaNhanVien()));
		try {
			Connection cons = DBConnect.getConnection();
